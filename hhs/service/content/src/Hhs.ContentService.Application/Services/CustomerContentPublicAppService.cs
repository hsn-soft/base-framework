...
using Hhs.ContentService.Domain.ContentDomain.Consts.Public;
...
) : ApplicationServiceBase(provider), ICustomerContentPublicAppService
{
    ...
    public async Task<GetOrCreateCustomerContentResponseDto> GetOrCreateAsync(GetOrCreateCustomerContentRequestDto input, CancellationToken cancellationToken = default)
    {
        ...
        else
        {
            ...
            var placedCustomerContent = await customerContentRepository.CreateAsync(
                ...
                operationStatus: CustomerContentOperationStates.CreatedWaitForNormalize,
            ...
            _logger.FrameworkInfoLog(LogHelper.Generate(
                ...
                facility: CustomerContentOperationFacilities.CUSTOMER_CONTENT_CREATED,
    ...
    public async Task CreateAdResultAsync(CreateContentAdResultDto input, CancellationToken cancellationToken = default)
    {
        ...
        if (input.FeedKey is not (CustomerContentOperationFacilities.ADS_ERROR
            or CustomerContentOperationFacilities.ADS_TIMEOUT
            or CustomerContentOperationFacilities.ADS_EMPTY
            or CustomerContentOperationFacilities.ADS_STARTED))
        {
            return;
        }
