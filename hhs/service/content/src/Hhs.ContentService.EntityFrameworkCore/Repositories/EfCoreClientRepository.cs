    ...
    : EfCoreGenericRepository<Client, Guid>(provider, dbContext), IClientRepository
{
        ...
        List<string> excludePathFilters = null)
    {
        ...
        {
            filters.AddRange(includePathFilters.Select(x => new ClientPathFilter(id: Guid.CreateVersion7(), tenantId: tenantId, clientId: id, pathFilterName: x, clientFilterType: ClientFilterTypes.IncludeFilter)).ToList());
        }

        if (excludePathFilters is { Count: > 0 })
        {
            filters.AddRange(excludePathFilters.Select(x => new ClientPathFilter(id: Guid.CreateVersion7(), tenantId: tenantId, clientId: id, pathFilterName: x, clientFilterType: ClientFilterTypes.ExcludeFilter)).ToList());
        }
        ...
        List<string> excludePathFilters = null)
    {
        ...
        if (includePathFilters is { Count: > 0 })
        {
            filters.AddRange(includePathFilters.Select(x => new ClientPathFilter(id: Guid.CreateVersion7(), tenantId: oldEntity.TenantId, clientId: id, pathFilterName: x, clientFilterType: ClientFilterTypes.IncludeFilter)).ToList());
        }

        if (excludePathFilters is { Count: > 0 })
        {
            filters.AddRange(excludePathFilters.Select(x => new ClientPathFilter(id: Guid.CreateVersion7(), tenantId: oldEntity.TenantId, clientId: id, pathFilterName: x, clientFilterType: ClientFilterTypes.ExcludeFilter)).ToList());
        }
