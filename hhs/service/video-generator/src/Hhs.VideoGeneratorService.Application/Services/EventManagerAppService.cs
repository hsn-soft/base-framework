...
public sealed class EventManagerAppService : ApplicationServiceBase, IEventManagerAppService
{
    ...
    public async Task EventReQueuedAsync(MessageEnvelope<ReQueuedEto> @event)
    {
        ...
        {
            case nameof(VideoRequestSendStartedEto):
            {
                if (skipReQueueOperation)
                {
                    await EventBus.PublishAsync(parentMessage: ParentIntegrationEvent, eventMessage: new SetVideoRequestStatusToFailedEto(
                        VideoRequestId: ((dynamic)originalEvent)?.VideoRequestId,
                        FailedReason: EventManagerOperationFacilities.EVENT_REQUEUED_FAILED + ": ReQueue Limit Error")
                    );
                }
                else
                {
                    await EventBus.PublishAsync(parentMessage: ParentIntegrationEvent, eventMessage: originalEvent as VideoRequestSendStartedEto);
                }

                break;
            }
            case nameof(VideoFileDownloadToLocalStartedEto):
            {
                if (skipReQueueOperation)
                {
                    await EventBus.PublishAsync(parentMessage: ParentIntegrationEvent, eventMessage: new SetVideoRequestStatusToFailedEto(
                ...
                {
                    await EventBus.PublishAsync(parentMessage: ParentIntegrationEvent, eventMessage: originalEvent as VideoFileDownloadToLocalStartedEto);
                }

                break;
            }
            case nameof(VideoFileUploadToStorageStartedEto):
            {
                if (skipReQueueOperation)
                {
                    await EventBus.PublishAsync(parentMessage: ParentIntegrationEvent, eventMessage: new SetVideoRequestStatusToFailedEto(
                ...
                {
                    await EventBus.PublishAsync(parentMessage: ParentIntegrationEvent, eventMessage: originalEvent as VideoFileUploadToStorageStartedEto);
                }
