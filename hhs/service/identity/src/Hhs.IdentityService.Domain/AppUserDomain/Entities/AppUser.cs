...
public sealed class AppUser : AuditedEntity<Guid>, ISoftDelete, IMultiTenant
{
    ...
    {
        string checkValue = LocalizedModelValidator.NotNullOrWhiteSpace(userName, $"{nameof(AppRole)}:{nameof(UserName)}", AppUserConsts.UserNameMaxLength);
        UserName = StringHelper.Minimize(StringHelper.ReplaceInvalidChars(checkValue));
        NormalizedUserName = StringHelper.Normalize(UserName);
    }

    internal void SetEmail(string email)
    {
        string checkValue = LocalizedModelValidator.NotNullOrWhiteSpace(email, $"{nameof(AppRole)}:{nameof(Email)}", AppUserConsts.EmailMaxLength);
        Email = StringHelper.Minimize(StringHelper.ReplaceInvalidChars(checkValue, isEmail: true));
    ...
    internal void SetLanguageCode(string defaultLanguage)
    {
        ...
        {
            string checkValue = LocalizedModelValidator.Length(defaultLanguage, $"{nameof(AppUser)}:{nameof(LanguageCode)}", AppUserConsts.LanguageCodeMaxLength);
            LanguageCode = StringHelper.Minimize(StringHelper.ReplaceInvalidChars(checkValue));
