...
public sealed class NormalizedAnalysisController(IServiceProvider provider, INormalizedAnalysisAppService normalizedAnalysisAppService) : BaseServiceController(provider)
{
    [HttpGet("{id:guid}")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<NormalizedAnalysisDto> GetNormalizedAnalysisAsync(Guid id, CancellationToken cancellationToken = default)
    ...
    [HttpPost("paged-list")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<PagedDataResultDto<NormalizedAnalysisDto>> GetPagedListAsync([FromBody] GetNormalizedAnalysisPaged pagedInput, CancellationToken cancellationToken = default)
        => await normalizedAnalysisAppService.GetPagedListAsync(pagedInput, cancellationToken);

    [HttpPost("filter-list")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<List<NormalizedAnalysisDto>> GetFilterListAsync([FromBody] GetNormalizedAnalysisFilter filterInput, CancellationToken cancellationToken = default)
