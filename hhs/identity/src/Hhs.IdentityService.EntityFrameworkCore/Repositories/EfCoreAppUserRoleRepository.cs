...
public class EfCoreAppUserRoleRepository : EfCoreGenericRepository<AppUserRole, Guid>, IAppUserRoleRepository
{
    ...
    public async Task<AppUserRole> CreateAsync(Guid userId, Guid roleId)
    {
        var draft = new AppUserRole(userId, roleId);
    ...
    public async Task<int> CreateManyAsync(Guid userId, List<Guid> roleIds)
    {
        List<AppUserRole> draftList = [];

        draftList.AddRange(roleIds.Select(roleId => new AppUserRole(userId, roleId)));
