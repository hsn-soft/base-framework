...
public sealed class SessionPermissionAppService : ApplicationServiceBase, ISessionPermissionAppService
{
    ...
    private readonly ICachePermissionGrantRepository _cachePermissionGrantRepository;
    ...
    public SessionPermissionAppService(IServiceProvider provider,
        ...
        ICachePermissionGrantRepository cachePermissionGrantRepository,
        IMenuPermissionAppService menuPermissionAppService) : base(provider)
    {
        ...
        _cachePermissionGrantRepository = cachePermissionGrantRepository;
    ...
    public async Task<SessionPermissionsDto> GetSessionPermissionsAsync(GetSessionPermissionsFilter filter)
    {
        ...
        var cachedPermissions = await _cachePermissionGrantRepository.GetSessionPermissionsAsync(filter.ClientKey, filter.RoleKeys, filter.UserKey);
        if (cachedPermissions == null)
        {
            throw new BaseHttpException((int)HttpStatusCode.RequestTimeout);
        }

        foreach (var groupItem in cachedPermissions.GroupBy(x => x.Name))
        {
    ...
    {
        var permissionGrants = await _permissionGrantRepository.GetListAsync(new ListQueryOptions<PermissionGrant>());
        if (permissionGrants is { Count: > 0 })
        {
            await _cachePermissionGrantRepository.SetPermissionsAsync(permissionGrants.Select(x => new CachePermissionGrant(x.Name, x.ProviderName, x.ProviderKey)).ToList());
            _logger.LogInformation("PERMISSION CACHE DB | {OperationStatus} => [ {PermissionGrantsCount} ]", "UPDATED", permissionGrants.Count);
        }
        else
        {
            await _cachePermissionGrantRepository.ClearPermissionsAsync();
            _logger.LogInformation("PERMISSION CACHE DB | {OperationStatus}", "CLEARED");
        }

        // Publish Permission Grants Update Integration Event for other microservices skip it wait periods for synchronization
