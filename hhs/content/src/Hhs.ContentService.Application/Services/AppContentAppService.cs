...
public sealed class AppContentAppService : ApplicationServiceBase, IAppContentAppService
{
    ...
    public async Task<PagedDataResultDto<AppContentDto>> GetPagedListAsync(GetAppContentsPaged pagedInput, CancellationToken cancellationToken = default)
    {
        ...
        pagedInput.SearchText = StringOperations.SlugKeyNormalize(pagedInput.SearchText);
        pagedInput.SlugKey = StringOperations.SlugKeyNormalize(pagedInput.SlugKey);
    ...
    public async Task<List<AppContentDto>> GetFilterListAsync(GetAppContentsFilter filterInput, CancellationToken cancellationToken = default)
    {
        ...
        filterInput.SlugKey = StringOperations.SlugKeyNormalize(filterInput.SlugKey);
    ...
    public async Task<List<AppContentSearchDto>> GetSearchListAsync(GetAppContentsSearch searchInput, CancellationToken cancellationToken = default)
    {
        ...
        searchInput.SearchText = StringOperations.SlugKeyNormalize(searchInput.SearchText);
    ...
    public async Task SetNormalizedResultAsync(Guid appContentId, Guid normalizedRequestId, bool isNormalizedSuccess, DateTime? releaseTime = null, string correlationId = null)
    {
        ...
        if (placed.OperationStatus == AppContentOperationStates.NormalizedWaitForVideoGenerationApprove)
        {
            _logger.FrameworkInfoLog(LogHelper.Generate(
                ...
                reference: new { placed.TenantId, placed.ClientId, RefContentId = placed.Id, placed.NormalizedRequestId },
                facility: AppContentOperationFacilities.APP_CONTENT_NORMALIZED_SUCCESS,
                correlationId: correlationId,
                exception: null
            ));

            var checkResult = await CheckVideoGenerationApproveRules(placed.TenantId, placed.ClientId, placed.ReleaseTime);
            if (checkResult.Key)
            {
    ...
    public async Task TrendVideoGenerationQueryAsync(TrendVideoGenerationQueryEto input, string correlationId = null)
    {
        ...
        long clientDailyTrendVideoHistoryCount = await _clientVideoGenerationHistoryRepository.GetClientVideoHistoryCountAsync(client.Id,
        ...
        {
            var contentIds = await _appContentRepository.GetClientDailyTrendContentIdsAsync(client.Id, client.DailyTrendVideoWaitStatisticHour);
                        ...
                        if (placed is { OperationStatus: AppContentOperationStates.VideoGenerationRejectedReturnAnalysisVideo })
                        {
                            ...
                            try
                            {
                                ...
                                _logger.FrameworkInfoLog(LogHelper.Generate(
                                    ...
                                    reference: new { placed.TenantId, placed.ClientId, AppContentId = placed.Id, placed.NormalizedRequestId },
                                    facility: AppContentOperationFacilities.APP_CONTENT_VIDEO_GENERATION_APPROVED,
    ...
    private async Task<KeyValuePair<bool, string>> CheckVideoGenerationApproveRules(Guid tenantId, Guid clientId, DateTime? releaseTime)
    {
        ...
        if (client == null)
        {
            throw new ClientNotFoundException(L, clientId.ToString());
        }
        ...
        long clientDailyDirectVideoHistoryCount = await _clientVideoGenerationHistoryRepository.GetClientVideoHistoryCountAsync(clientId,
