...
) : ApplicationServiceBase(provider), IAppContentPublicAppService
{
    ...
    public async Task<AppContentDto> GetAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        var item = await appContentRepository.GetSingleOrDefaultAsync(x => x.Id == id, cancellationToken: cancellationToken);
        if (item == null)
        {
            throw new AppContentNotFoundException(L, id.ToString());
        }

        return Mapper.Map<AppContent, AppContentDto>(item);
    }
    ...
    public async Task<GetOrCreateAppContentResponseDto> GetOrCreateAsync(GetOrCreateAppContentRequestDto input, CancellationToken cancellationToken = default)
    {
        ...
        input.DomainName = input.DomainName.ToLower(new CultureInfo("en-US"));
        ...
        // Check Client
        bool clientCheckResult = await clientRepository.ExistsAsync(x =>
        ...
        string normalizedSlugKey = input.ContentKey.Replace("/", "_");
        var contentStatusModel = await appContentRepository.GetFirstOrDefaultAsync<AppContentStatusDto>(
        ...
        {
            // Get Client Details
            var clientCheck = await clientRepository.GetSingleOrDefaultAsync<ClientCheckDto>(
                x => x.Id == input.CustomerId.Value,
                Mapper.ConfigurationProvider, cancellationToken: cancellationToken);
            if (clientCheck == null)
            {
    ...
    public async Task CreateAdResultAsync(CreateContentAdResultDto input, CancellationToken cancellationToken = default)
    {
        ...
        switch (input.ContentType)
        {
            case AppContentPublicType.APP_CONTENT:
                {
                    var content = await appContentRepository.GetSingleOrDefaultAsync(x => x.Id == input.ContentId, cancellationToken: cancellationToken);
                    if (content == null)
                    {
            ...
            case AppContentPublicType.ANALYSIS_CONTENT:
                {
                    var analysisContent = await analysisContentRepository.GetSingleOrDefaultAsync(x => x.Id == input.ContentId, cancellationToken: cancellationToken);
                    if (analysisContent == null)
                    {
                        throw new AppContentNotFoundException(L, input.ContentId.ToString())
                            .WithData(nameof(input.ContentId), input.ContentId);
