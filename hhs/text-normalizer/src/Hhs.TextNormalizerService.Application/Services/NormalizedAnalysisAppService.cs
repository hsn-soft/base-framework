...
public sealed class NormalizedAnalysisAppService : ApplicationServiceBase, INormalizedAnalysisAppService
{
    ...
    public async Task<PagedDataResultDto<NormalizedAnalysisDto>> GetPagedListAsync(GetNormalizedAnalysisPaged pagedInput, CancellationToken cancellationToken = default)
    {
        ...
        pagedInput.SearchText = StringOperations.Minimize(pagedInput.SearchText);
        pagedInput.DomainName = StringOperations.Minimize(pagedInput.DomainName);
    ...
    public async Task<List<NormalizedAnalysisDto>> GetFilterListAsync(GetNormalizedAnalysisFilter filterInput, CancellationToken cancellationToken = default)
    {
        ...
        filterInput.DomainName = StringOperations.Minimize(filterInput.DomainName);
    ...
    public async Task CreateAsync(AnalysisContentNormalizedStartedEto input, string correlationId = null)
    {
        ...
        var analysisReferenceList = normalizedRequestList.Select(x => new AnalysisReferenceModel { AppContentId = x.AppContentId, NormalizedRequestId = x.Id, AnalysisDataModel = Mapper.Map<ScrapingContentDataModel, AnalysisDataModel>(x.ScrapingContentData) }).ToList();
        ...
        try
        {
            if (normalizedAnalysisItem.AnalysisReferenceList is { Count: > 0 })
            {
                ...
                var outlineInput = normalizedAnalysisItem.AnalysisReferenceList.Select(x => new OutlineRequestDto
                {
                    ...
                    OutlineInput = clientSettings.NormalizerSetting.IsForceContentDetailInAnalyseActive ? x.AnalysisDataModel.Details : x.AnalysisDataModel.Title.Replace(":", "") + " : " + (x.AnalysisDataModel.Spot ?? x.AnalysisDataModel.Details)
                }).ToList();
                ...
                {
                    response = await _outlineProvider.OutlineAsync(outlineInput);
                ...
                if (response is { Count: > 0 })
                {
                    ...
                    {
                        response[0].OutlinedData = (introResponse.OutlinedData) + ", " + response[0].OutlinedData;
                    }

                    //Manipulate last content
                    if (!string.IsNullOrWhiteSpace(outroResponse?.OutlinedData))
                    {
                        response[^1].OutlinedData = response[^1].OutlinedData + ", " + (outroResponse.OutlinedData ?? string.Empty);
                    }
