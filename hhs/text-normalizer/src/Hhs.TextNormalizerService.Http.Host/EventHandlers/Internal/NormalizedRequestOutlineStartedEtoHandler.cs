...
public class NormalizedRequestOutlineStartedEtoHandler : IIntegrationEventHandler<NormalizedRequestOutlineStartedEto>
{
    private readonly IAppConsoleLogger _logger;
    private readonly INormalizedRequestAppService _normalizedRequestAppService;

    public NormalizedRequestOutlineStartedEtoHandler(IAppConsoleLogger logger,
    ...
    public async Task HandleAsync(MessageEnvelope<NormalizedRequestOutlineStartedEto> @event)
    {
        _logger.LogDebug("EVENT HANDLING | [ {EventName} ] => CorrelationId[{CorrelationId}], MessageId[{MessageId}], RelatedMessageId[{RelatedMessageId}]",
            nameof(NormalizedRequestOutlineStartedEto)[..^"Eto".Length],
            @event.CorrelationId ?? string.Empty,
            @event.MessageId.ToString(),
            @event.ParentMessageId != null ? @event.ParentMessageId.Value.ToString() : string.Empty);

        _normalizedRequestAppService.SetParentIntegrationEvent(@event);
        await _normalizedRequestAppService.OutlineAsync(@event.Message.NormalizedRequestId);
    }
}