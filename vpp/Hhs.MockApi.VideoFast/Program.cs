...
{
    var (trackingId, fileUrl) = await VideoFastService.GenerateVideoAsync(request.AudioUrls, mockFilesDir, ct);
    return Results.Ok(new { provider = "video-fast", remoteFileUrl = fileUrl, processingMs = 5000, trackingId });
});
...
{
    public static async Task<(string TrackingId, string FileUrl)> GenerateVideoAsync(List<string> audioUrls, string mockFilesDir, CancellationToken cancellationToken)
    {
