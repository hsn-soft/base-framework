...
public static class ServiceCollectionExtensions
{
    extension(IServiceCollection services)
    {
        public IServiceCollection AddMicroserviceHosting(IConfiguration configuration, Type type)
        {
                    ...
                    if (!env.IsProduction())
                    {
                        messages.AddRange(
                            context.ModelState
                                ...
                                {
                                    string message = string.IsNullOrWhiteSpace(e.ErrorMessage)
                                        ? "Invalid value."
                                        : e.ErrorMessage;
            ...
            services.AddTransient<ICachePermissionAssignmentRepository, CachePermissionAssignmentRepository>();
            services.AddHostedService<SynchServicePermissionStoreBackgroundService>();

            services.AddTransient<ICachePermissionConstraintAssignmentRepository, CachePermissionConstraintAssignmentRepository>();
            services.AddHostedService<SynchServicePermissionConstraintStoreBackgroundService>();
