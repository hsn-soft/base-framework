...
public class CustomerContentNormalizedResultEtoHandler(
    IAppConsoleLogger logger,
    ICustomerContentAppService customerContentAppService
) : IIntegrationEventHandler<CustomerContentNormalizedResultEto>
{
    ...
    public async Task HandleAsync(MessageEnvelope<CustomerContentNormalizedResultEto> @event)
    {
        _logger.LogDebug("EVENT HANDLING | [ {EventName} ] => CorrelationId[{CorrelationId}], MessageId[{MessageId}], RelatedMessageId[{RelatedMessageId}]",
            nameof(CustomerContentNormalizedResultEto)[..^"Eto".Length],
            @event.CorrelationId ?? string.Empty,
            @event.MessageId.ToString(),
            @event.ParentMessageId != null ? @event.ParentMessageId.Value.ToString() : string.Empty);

        _customerContentAppService.SetParentIntegrationEvent(@event);
        await _customerContentAppService.SetCustomerContentNormalizedResultAsync(@event.Message.CustomerContentId, @event.Message.ContentNormalizedRequestId,
