...
public sealed class InStyleScrapingService(Uri targetUri, IAppConsoleLogger logger, IPuppeteerBrowser puppeteer)
    : BaseScrapingService(targetUri, logger, puppeteer)
{
    private const string TitleScript =
        "() => Array.from(document.querySelectorAll('.container .post-title'), (e) => e.innerText)";

    private const string ReleaseTimeScript =
        "() => Array.from(document.querySelectorAll('meta[property=\"article:published_time\"]'), (e) => { return e.content }).filter((item) => item !== '' && item !== ' ').slice(0, 1)";

    private const string SpotScript = "";

    private const string DetailsScript =
        "() => Array.from(document.querySelectorAll('.container .entry-content'), (e) => ({targetText: e.innerText,filterValue: e.parentNode.className.trim()})).filter(item => item.filterValue === 'single56__body')";

    private const string ImageScript =
        "() => Array.from(document.querySelectorAll('.container .post-thumbnail img'), (e) => e.getAttribute('src'))";
