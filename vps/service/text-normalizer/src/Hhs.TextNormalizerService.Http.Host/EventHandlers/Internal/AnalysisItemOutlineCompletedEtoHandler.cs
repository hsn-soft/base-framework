...
) : IIntegrationEventHandler<AnalysisItemOutlineCompletedEto>
{
    ...
    public async Task HandleAsync(MessageEnvelope<AnalysisItemOutlineCompletedEto> @event)
    {
        _logger.LogDebug("EVENT HANDLING | [ {EventName} ] => CorrelationId[{CorrelationId}], MessageId[{MessageId}], RelatedMessageId[{RelatedMessageId}]",
            nameof(AnalysisItemOutlineCompletedEto)[..^"Eto".Length],
            @event.CorrelationId ?? string.Empty,
            @event.MessageId.ToString(),
            @event.ParentMessageId != null ? @event.ParentMessageId.Value.ToString() : string.Empty);

        _normalizerOperationAppService.SetParentIntegrationEvent(@event);
        await _normalizerOperationAppService.CompleteAnalysisItemOutlineAsync(@event.Message);
    }
}