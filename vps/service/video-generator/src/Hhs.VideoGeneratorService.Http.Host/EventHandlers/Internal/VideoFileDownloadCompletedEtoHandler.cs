...
) : IIntegrationEventHandler<VideoFileDownloadCompletedEto>
{
    ...
    public async Task HandleAsync(MessageEnvelope<VideoFileDownloadCompletedEto> @event)
    {
        ...
        await _videoOperationAppService.HandleVideoDownloadCompletedAsync(@event.Message);
    }
}