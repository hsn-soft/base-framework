...
) : ApplicationEventHandlerBase<VideoFileDownloadCompletedEto>(inboxStore, logger, videoOperationAppService)
{
    protected override async Task ExecuteAsync(MessageEnvelope<VideoFileDownloadCompletedEto> @event, CancellationToken cancellationToken)
        => await videoOperationAppService.HandleVideoDownloadCompletedAsync(@event.Message, cancellationToken);
}