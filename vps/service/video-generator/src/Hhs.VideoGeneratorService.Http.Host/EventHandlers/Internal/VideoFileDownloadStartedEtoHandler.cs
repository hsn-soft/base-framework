...
) : ApplicationEventHandlerBase<VideoFileDownloadStartedEto>(inboxStore)
{
    ...
    protected override async Task ExecuteAsync(MessageEnvelope<VideoFileDownloadStartedEto> @event, CancellationToken cancellationToken)
    {
        ...
        await _videoOperationAppService.DownloadVideoFileAsync(@event.Message, cancellationToken);
    }
}