...
) : ApplicationEventHandlerBase<VideoProviderPollingStartedEto>(inboxStore, logger, videoOperationAppService)
{
    protected override async Task ExecuteAsync(MessageEnvelope<VideoProviderPollingStartedEto> @event, CancellationToken cancellationToken)
        => await videoOperationAppService.ScheduleVideoProviderPollingAsync(@event.Message, cancellationToken);
}