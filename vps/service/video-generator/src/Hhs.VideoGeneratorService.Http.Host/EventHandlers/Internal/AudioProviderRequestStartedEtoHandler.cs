...
) : IIntegrationEventHandler<AudioProviderRequestStartedEto>
{
    ...
    public async Task HandleAsync(MessageEnvelope<AudioProviderRequestStartedEto> @event)
    {
        ...
        await _videoOperationAppService.StartAudioProviderRequestAsync(@event.Message);
    }
}