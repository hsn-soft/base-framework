...
) : IIntegrationEventHandler<VideoProviderCompletedEto>
{
    ...
    public async Task HandleAsync(MessageEnvelope<VideoProviderCompletedEto> @event)
    {
        ...
        await _videoOperationAppService.HandleVideoProviderCompletedAsync(@event.Message);
    }
}