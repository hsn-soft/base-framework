...
) : IIntegrationEventHandler<VideoOperationStartedEto>
{
    ...
    public async Task HandleAsync(MessageEnvelope<VideoOperationStartedEto> @event)
    {
        ...
        await _videoOperationAppService.HandleVideoOperationStartedAsync(@event.Message, @event.MessageId);
    }
}