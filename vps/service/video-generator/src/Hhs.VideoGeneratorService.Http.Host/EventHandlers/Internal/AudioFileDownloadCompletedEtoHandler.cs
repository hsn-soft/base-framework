...
) : ApplicationEventHandlerBase<AudioFileDownloadCompletedEto>(inboxStore, logger, videoOperationAppService)
{
    protected override async Task ExecuteAsync(MessageEnvelope<AudioFileDownloadCompletedEto> @event, CancellationToken cancellationToken)
        => await videoOperationAppService.HandleAudioDownloadCompletedAsync(@event.Message, cancellationToken);
}