...
) : IIntegrationEventHandler<AudioFileUploadCompletedEto>
{
    ...
    public async Task HandleAsync(MessageEnvelope<AudioFileUploadCompletedEto> @event)
    {
        ...
        await _videoOperationAppService.HandleAudioUploadCompletedAsync(@event.Message);
    }
}